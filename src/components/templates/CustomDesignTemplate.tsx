  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ position: 'absolute', bottom: '-40px', left: '-30px', width: '80px', height: '80px', borderRadius: '50%', background: 'rgba(255,255,255,0.08)' }} />
        
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
            <span style={{ fontSize: '24px' }}>{content.icons?.[0] || '✨'}</span>
            <span style={{ fontSize: '11px', fontWeight: 600, color: 'rgba(255,255,255,0.8)', textTransform: 'uppercase', letterSpacing: '2px' }}>
              AI Generated Design
            </span>
          </div>
          <h1
          ...
          {content.subtitle && (
            <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.85)', margin: 0, lineHeight: 1.4, fontWeight: 400 }}>
              {content.subtitle}
            </p>
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: '8px', padding: '16px 20px' }}>
          {content.statistics.slice(0, 3).map((stat, i) => (
            ...
            >
              ...
              <div style={{ fontSize: '22px', fontWeight: 800, color: [primaryColor, secondaryColor, tertiaryColor][i], lineHeight: 1 }}>
                {stat.prefix}{stat.value}{stat.suffix}
              </div>
              <div style={{ fontSize: '10px', color: '#6b7280', fontWeight: 500 }}>{stat.label}</div>
            </div>
          ))}
        </div>
      )}
      ...
      >
        {content.sections.slice(0, 4).map((section, i) => (
          ...
          >
            <div style={{ position: 'absolute', top: 0, left: 0, width: '4px', height: '100%', background: `linear-gradient(${[primaryColor, secondaryColor, tertiaryColor][i % 3]}, ${[secondaryColor, tertiaryColor, primaryColor][i % 3]})` }} />
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px', paddingLeft: '4px' }}>
              <span style={{ fontSize: '18px' }}>{section.icon}</span>
              {section.title && (
                <h3 style={{ fontSize: '13px', fontWeight: 700, color: '#1f2937', margin: 0 }}>{section.title}</h3>
              )}
            </div>
            {section.content && (
              <p style={{ fontSize: '11px', color: '#4b5563', margin: 0, lineHeight: 1.6, paddingLeft: '4px' }}>{section.content}</p>
            )}
          </div>
        ))}
      </div>
      ...
      {/* Brand */}
      <div style={{ position: 'absolute', bottom: '6px', right: '12px', fontSize: '7px', color: 'rgba(0,0,0,0.15)', letterSpacing: '1.5px', fontWeight: 600 }}>
        AI DESIGN
      </div>
    </div>
  );
